...
class MqttTransportServer(MqttTransportBase):
    ...
    async def create_session(self, mcp_client_id: str, msg: mqtt.MQTTMessage):
        ...
        ## 1. (msg) --> MqttBroker --> MqttTransportServer --[raw_read_stream]--> conversion
        ##    --[session_read_stream]--> MCPSession
        ## 2. MqttBroker <-- MqttTransportServer <--[raw_write_stream_reader]-- conversion
        ##    <--[session_write_stream]-- MCPSession <-- (msg)
        ...
        # Start session with SessionMessage streams
